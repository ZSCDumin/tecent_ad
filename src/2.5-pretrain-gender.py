...
def train_epoc(trn, model, criterion, optimizer, scheduler):
    ...
    for label, data_x, data_len in pbar:
        ...
        loss.backward()
        optimizer.step()
        train_acc += (output.argmax(1) == label[:,1]).sum().item()
...
def val_epoc(tst, model, criterion):
    ...
    for label, data_x, data_len in tst:
        ...
        with torch.no_grad():
            ...
            loss = criterion(output, label[:,1])
            loss += loss.item()
            acc += (output.argmax(1) == label[:,1]).sum().item()
    return loss, acc
...
if __name__ == "__main__":
    ...
    train_ds = dataset.seq_dataset(
        train_df, wv_path="data/w2v/512/",seq_len=SEQ_LEN, cols_seq=COLS,)
    val_ds = dataset.seq_dataset(
        val_df, wv_path="data/w2v/512/",seq_len=SEQ_LEN, cols_seq=COLS,)
    ...
    for epoch in range(N_EPOCHS):
        ...
        loss_plt1.append(valid_acc/len_val)
        torch.save(model.state_dict(), "512-gender-"+str(epoch)+".pth")
    print(loss_plt,loss_plt1)
