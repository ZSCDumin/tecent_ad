...
def train_epoc(trn, model, criterion, optimizer, scheduler):
    ...
    for label, data_x, data_len in pbar:
        ...
        train_acc += (output.argmax(1) == label[:,0]).sum().item()
...
def val_epoc(tst, model, criterion):
    ...
    loss = 0
    acc = 0
    for label, data_x, data_len in tqdm(tst):
        label = label.to(device)
        data_len = data_len.to(device)
        for i in range(len(data_x)):
            # data_x[i]=rnn_utils.pack_padded_sequence(data_x[i].to(device),data_len,batch_first=True,enforce_sorted=False)
        ...
        with torch.no_grad():
            ...
            loss = criterion(output, label[:,0])
            loss += loss.item()
            acc += (output.argmax(1) == label[:,0]).sum().item()
    return loss, acc
...
if __name__ == "__main__":
    ...
    train_df=train_df.sample(frac=0.2,random_state=222222)
    train_df.reset_index(drop=True, inplace=True)
    val_df.reset_index(drop=True, inplace=True)
    # dataset
    train_ds = dataset.seq_dataset(
        train_df, wv_path="data/w2v/",seq_len=SEQ_LEN, cols_seq=COLS,)
    val_ds = dataset.seq_dataset(
        val_df, wv_path="data/w2v/",seq_len=SEQ_LEN, cols_seq=COLS,)
    ...
    optimizer = torch.optim.SGD(model.parameters(), momentum=0.9,lr=0.01) # Adam 0.01
    ...
    for epoch in range(N_EPOCHS):
        ...
        torch.save(model.state_dict(), "pt-"+str(epoch)+".pth")
    print(loss_plt)
