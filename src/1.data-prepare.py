...
def reduce_mem_usage(df, verbose=True):
    ...
    return df
print("训练")
train_dir = "data/train_preliminary/"
test_dir = "data/test/"
...
print("原始的数据保存")
click_all.to_pickle("data/click-raw.pkl")
print("0填充")
...
click_all.to_pickle("data/click-fill0.pkl")
print("载入用户标签")
user_label = pd.read_csv(train_dir + "user.csv")
print("年龄和性别合并")
...
data_all.info()
data_all.to_pickle("data/all.pkl")


print("advertiser_id和product_id")
data_all["adv_pro"]=data_all["advertiser_id"].map(str)+":"+data_all["product_id"].map(str)
...
for col in cate_cols:
    ...
    torch.save(le,"data/"+col+".pkl")

print(data_all.info())
#全部编码后的文件
data_all.to_pickle("data/all-encoded.pkl")
