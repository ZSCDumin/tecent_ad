...
class baseline_model(nn.Module):
    def __init__(self, emb_dims, hidden_size,seq_len,attn_size,num_class,device,emb_dropout=0.,bidirectional=False,num_layers=1):
        super().__init__()
        ...
        self.lstm=nn.LSTM(self.no_of_embs,num_layers=num_layers,hidden_size=hidden_size,batch_first=True,bidirectional=bidirectional,dropout=emb_dropout)
    ...
    def forward(self,data,data_len):
        ...
        exit()
        '''
        out = [emb_layer(data[i]) for i,emb_layer in enumerate(self.emb_layers)]
        out = torch.cat(out, 2)
